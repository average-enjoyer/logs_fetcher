...
To do that, you may fill out the default settings in:
* `C:\Users\<Username>\AppData\Roaming\log_harvester\settings.json` - if you run `log_harvester.exe` on Windows OR
* `/home/<USERNAME>/.config/log_harvester/settings.json` - if you run `log_harvester_linux_x64` on Linux
* `src\log_harvester\settings.json` - if you run code manually.<br>
**Note: The settings file for Linux and Windows appears only after the first launch of binary.**
...
## How to install
* Windows: Download [log_harvester.exe](https://github.com/average-enjoyer/LogHarvester/releases)
* Linux (x64): Download [log_harvester_linux_x64](https://github.com/average-enjoyer/LogHarvester/releases) 
* or [run the code](README.MD#how-to-run-code)

## How to run code

The project is extremely simple.
Just recreate the environment after cloning and run `main.py` *(Python3.12+)*
```
python -m venv venv
